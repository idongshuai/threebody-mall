	...
	var canvg_1 = createCommonjsModule(function (module) {
		...
		function build(opts) {
			...
			if (svg.opts['log'] == true && typeof console != 'undefined') {
				svg.log = function (msg) {  }; // eslint-disable-line no-console
			}
			...
			svg.Element.image = function (node) {
				...
				if (!isSvg) {
					...
					this.img.onerror = function () {
						Util.globalTopTip("下载失败，文件中存在不授信或者失效的图片,<br>请检查后重试，<a href='#'>点击查看</a>", "top_error", 8000, $("#export_dialog"), true);
			...
			svg.CreateElement = function (node) {
				if (node.nodeName == "svg") {
					if ($(node).attr('viewBox')) {
						var viewBox = $(node).attr('viewBox').split(' ')
						$(node).attr({"width" :viewBox[2] ,"height" :viewBox[3]});
						$(node).removeAttr('viewBox')
					}
				}

...
var Schema = {
	...
	shapeDefaults: {
		...
		attribute: {
			...
			collapsable: false,
			collapsed: false,
			//collapseBy: "e43f56642df", //被哪个图形隐藏，可能是被subProcess折叠
			//collapseW: 0, //收缩以前的宽
			//collapseH: 0, //收缩以前的高
			markerOffset: 5
		},
...
//		},
	...
	addTheme: function(theme){
		Schema.themes[theme.name] = theme;
	},
	/**
	...
	initShape: function(shape){
		...
		for(var key in this.shapeDefaults){
			if(key == "attribute"){
				...
				result.lineStyle = this.extend({}, shape.lineStyle);
			}else if(key == "fillStyle"){
				result.fillStyle = this.extend({}, shape.fillStyle);
			}else if(key == "fontStyle"){
				result.fontStyle = this.extend({}, shape.fontStyle);
				...
				result.theme = this.extend({}, shape.theme);
			}else{
				if(typeof shape[key] != "undefined"){
					result[key] = shape[key];
				}else{
		...
		//直线添加文件定义
		if(shape.name.indexOf('linker_shape_') != -1 && shape.element){
			result.element = shape.element;
		}
		return result;
	},
	/**
	...
	initShapeFunctions: function(shape){
		...
		var pathEval = "shape.getPath = function(){" +
				"var color = [255,255,255];var fillStyle = Utils.getShapeFillStyle(this.fillStyle, false);if(fillStyle.color && fillStyle.color.length > 0){color = fillStyle.color.split(',');}" +
				"var r = color[0]; var g = color[1]; var b = color[2];" +
				"var w = this.props.w; var h = this.props.h;var lineStyle = Utils.getShapeLineStyle(this.lineStyle, false); var lineWidth = lineStyle.lineWidth; ";
...
var SchemaHelper = {
	 ...
	 * @param {} path
	 */
	constructPathFunBody: function(path, name){
		if(name && name.indexOf("grid") == 0){
			var path_ = [];
		...
		for(var i = 0; i < path.length; i++){
			...
			//一节路径
			pathEval += "{";
			if(cmd.fillStyle && $.isEmptyObject(cmd.fillStyle) == false){
			...
			//表格 行 和 列
			if(typeof cmd.row != "undefined"){
				var row = "row: " + cmd.row + ",";
				pathEval += row;
			}
			if(typeof cmd.column != "undefined"){
				var column = "column: " + cmd.column + ",";
				pathEval += column;
			}
			pathEval += "actions:["
			//一节路径中的每个指令
			...
			for (var j = 0; j < actions.length; j++) {
				...
				}else if(action.action == "rect"){
					pathEval += ",x:" + action.x + ",y:" + action.y + ",w:" + action.w + ",h:" + action.h;
				}
				pathEval += "}";

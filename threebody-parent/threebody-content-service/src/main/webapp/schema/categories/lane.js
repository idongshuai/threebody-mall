...
Schema.addCategory({
	name: "lane",
	text: "<@i18n resource='schema.category.lane'/>"
});
...
Schema.addShape({
	name: "verticalPool",
	title: "<@i18n resource='schema.lane.verticalpool'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false,
...
Schema.addShape({
	name: "verticalLane",
	title: "<@i18n resource='schema.lane.verticallane'/>",
	category: "lane",
	attribute: {
		container: true,
		rotatable: false,
	...
	onCreated: function(pool, isSavePool){
		//寻找重叠的泳池
		var pool = pool || getPool(this, "verticalPool");
		if(pool == null){
			//没有找到重叠的泳池，则创建一个泳池
			...
			for (var i = 0; i < pool.children.length; i++) {
				...
				if(child.name == "verticalLane"){
					...
					if(child.props.x > lastLane.props.x){
						lastLane = child;
					}
					laneCount++;
			...
			if(lastLane){
				this.fillStyle = lastLane.fillStyle;
				this.fontStyle = lastLane.fontStyle;
				this.props.w = lastLane.props.w;
				this.lineStyle = lastLane.lineStyle;
			}
			this.props.x = x;
...
Schema.addShape({
	...
	title: "<@i18n resource='schema.lane.verticalseparator.bar'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false,
...
Schema.addShape({
	...
	title: "<@i18n resource='schema.lane.horizontalseparator'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false
	...
	textBlock: [{position: {x: 0, y: 5, w: 20, h: "h-10"}, text: "<@i18n resource='schema.lane.separator.stage'/>"}],
	anchors: [],
	resizeDir: ["b"],
	path: [
		{
...
Schema.addShape({
	...
	title: "<@i18n resource='schema.lane.horizontalseparator.bar'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false,
...
Schema.addShape({
	...
	title: "<@i18n resource='schema.lane.verticalseparator'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false
	...
	textBlock: [{position: {x: 5, y: 0, w: "w-10", h: 20}, text: "<@i18n resource='schema.lane.separator.stage'/>"}],
	anchors: [],
	resizeDir: ["r"],
	path: [
		{
...
Schema.addShape({
	name: "horizontalPool",
	title: "<@i18n resource='schema.lane.horizontalpool'/>",
	category: "lane",
	attribute: {
		rotatable: false,
		linkable: false,
...
Schema.addShape({
	name: "horizontalLane",
	title: "<@i18n resource='schema.lane.horizontallnae'/>",
	category: "lane",
	attribute: {
		container: true,
		rotatable: false,
	...
	onCreated: function(pool, isSavePool){
		//寻找重叠的泳池
		var pool = pool || getPool(this, "horizontalPool");
		if(pool == null){
			//没有找到重叠的泳池，则创建一个泳池
			...
			for (var i = 0; i < pool.children.length; i++) {
				...
				if(child.name == "horizontalLane"){
					...
					if(child.props.y > lastLane.props.y){
						lastLane = child;
					}
					laneCount++;
